...
- **Path Planning** ── Implement Boustrophedon Cell Decomposition Path Coverage Algorithm and save the waypoints.
- **Path Following MAVSDK Script** ── MAVSDK script to arm, takeoff and follow those waypoints generated by BCD Algorithm.
- **Plotting the Difference** ── Plot the actual vs planned path
- **Identify 2 Weaknesses** ── Identify at least two weaknesses or edge cases in the current single-agent implementation and document them as considerations for future multi-agent swarm extension.

### Week 07 — Path Planning Algorithm implementation in multiple UAVs & Model Deploy on Edge Computer
- **Path Planning** ── Implement Boustrophedon Cell Decomposition Path Coverage Algorithm and save the waypoints.
- **Partitioning the coverage area** ── Partitioning the coverage area for into 3 regions for 3 UAVs.
- **Running the UAVs concurrently** ── MAVSDK script to arm, takeoff and follow those waypoints generated by BCD Algorithm by all UAVs in thier respected region.
- **Collision Avoidance** ── Implement basic collision avoidance when drones approach within 5m.
